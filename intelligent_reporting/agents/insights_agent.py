    ...
    def run(
        ...
        if offline_mode:
            ...
            try:
                ...
                print(
                    f"[{self.__class__.__name__}] 'langchain_openai' not found. Forcing offline mode (fallback logic required)."
                )
                # Insights Fallback is weird, it uses ChatOllama or FallbackManager?
                # Existing code line 32 used ChatOllama. Let's use that.
                from langchain_ollama import ChatOllama
