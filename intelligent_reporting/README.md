...
The agents module (`intelligent_reporting.agents`) contains the reasoning engines that drive the analysis pipeline. These agents utilize LLMs (Large Language Models) to perform semantic understanding, planning, code generation, and insight extraction.

## Available Agents

### 1. Metadata Agent
**Purpose:** Understands the semantic meaning of dataset columns.
- **Input:** Sample data, inferred schema.
- **Output:** Enriched metadata with semantic types and column descriptions.

### 2. Supervisor Agent
**Purpose:** Acts as the lead data analyst.
- **Input:** Dataset metadata.
- **Output:** A structural analysis plan consisting of specific, executable tasks (e.g., "Analyze monthly sales trends", "Identify correlation between price and demand").

### 3. Assistant Agent
**Purpose:** The coding specialist.
- **Input:** A single task from the Supervisor's plan.
- **Output:** Executable Python code (Pandas/Matplotlib) that generates the requested artifact (chart or summary).

### 4. Insights Agent
**Purpose:** The business analyst.
- **Input:** The chart or summary generated by the Assistant Agent.
- **Output:** Human-readable insights, key findings, and actionable recommendations based on the visual evidence.

## Orchestration

Agents are typically orchestrated via the `backend` service, which manages state and context. However, they can be instantiated directly for custom pipelines:

supervisor = SupervisorAgent()
plan = supervisor.run(sample_data=..., description=...)
```

## Roadmap

- [ ] **More Connectors**: Support for Notion, AirTable, and GraphQL APIs.
- [ ] **Advanced Reasoning**: Implement "Reflexion" patterns for agents to self-correct code errors.
- [ ] **Vector Memory**: Integrate RAG (Retrieval Augmented Generation) for querying large documentation or past reports.
- [ ] **Custom Tooling**: Allow users to define custom Python tools that agents can invoke.
